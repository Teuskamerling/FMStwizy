...
DRESULT disk_read (BYTE drv, BYTE *buff, DWORD sector, BYTE count)
{
	...
	while (count--)
	{
		// read operation should not be interrupted so start a critical section
		taskENTER_CRITICAL();
		sdstatus = SD_ReadBlock((BYTE *)scratch, (uint32_t )((sector + count) * SD_BLOCKSIZE), SD_BLOCKSIZE);
		// read done so leave the critical section
		taskEXIT_CRITICAL();
		if (sdstatus != SD_OK)
		{
			return RES_ERROR;
...
DRESULT disk_write(BYTE drv, const BYTE *buff, DWORD sector, BYTE count)
{
	...
	while (count--)
	{
		...
		// write operation should not be interrupted so start a critical section
		taskENTER_CRITICAL();
		sdstatus = SD_WriteBlock((BYTE *)scratch, (uint32_t )((sector + count) * SD_BLOCKSIZE), SD_BLOCKSIZE);
		// write done so leave the critical section
		taskEXIT_CRITICAL();
...
{
	/* Declare struct to hold date and time */
	...
	/* Check if RTC is implemented by checking the year and date */
	if( DateStruct.RTC_Year == 0 && DateStruct.RTC_Date == 1 && TimeStruct.RTC_Seconds == 0 &&
	TimeStruct.RTC_Minutes == 0 && TimeStruct.RTC_Hours == 0 ){
		/* No RTC support. Return a fixed value 1980/0/0 0:0:0 */
		fatTime =  ((DWORD)(1980 - 1980) << 25)	/* Y */
		| ((DWORD)0  << 21)       		/* M */
		| ((DWORD)0 << 16)       			/* D */
		| ((DWORD)0  << 11)       		/* H */
		| ((DWORD)0  << 5)        		/* M */
		...
		fatTime =  ((DWORD)((2000 + DateStruct.RTC_Year) - 1980) << 25)	/* Y */
		| ((DWORD)DateStruct.RTC_Month  << 21)       				/* M */
		| ((DWORD)DateStruct.RTC_Date << 16)       				/* D */
		| ((DWORD)TimeStruct.RTC_Hours  << 11)       				/* H */
		| ((DWORD)TimeStruct.RTC_Minutes  << 5)        			/* M */
		| ((DWORD)TimeStruct.RTC_Seconds  >> 1);       			/* S */
	}
	return fatTime;
}
